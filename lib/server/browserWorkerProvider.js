...
var getApi = function(provider){
	...
	api.getPendingWorkersSize = function() {
	    return Object.keys(provider.pendingWorkers).length;
    };
	
	api.name = provider.name;
...
BrowserWorkerProvider.prototype.messageHandler = function(message){
	...
	if (message.length === 3) {
		var tempMsg = message[2] && message[2][1];
		if (tempMsg) {
			tempMsg.name && (tempMsg.name = decodeURI(tempMsg.name));
			tempMsg.suiteName && (tempMsg.suiteName = decodeURI(tempMsg.suiteName));
		}
	}
